            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    ...
                    <div style="width:85%;margin: auto;"
                        ...
                        <a href="{{ route('DinerType.create') }}" class="btn "
                            ...
                            <span class="d-flex align-items-center mx-0"><i class="material-icons">&#xE147;</i><span
                                    class="ml-1">新增餐館類型</span></span>
                        </a>
                        <form action="{{ route('DinerType_search') }}" method="GET" class="">
                        ...
                        <tbody>
                            <tr>
                                @foreach ($DinerTypes as $DinerType)
                                    <td data-label="編號"> {{ $DinerType->dt_no }}</td>
                                    <td data-label="名稱"> {{ $DinerType->dt_typename }}</td>
                                    <td data-label="名稱"> {{ $DinerType->dt_sort }}</td>
                                    <td data-label="操作">
                                        <a href="{{ route('DinerType.show', $DinerType->id) }}" class="show mx-1">
                                            <i class="fa-sharp fa-solid fa-eye " style="color:#36304A;"
                                                data-toggle="tooltip" title="檢視"></i></a>


                                        <a href="{{ route('DinerType.edit', $DinerType->id) }}" class="edit mx-1">
                                            <i class="fa-solid fa-pen-to-square" style="color:#36304A;"
                                                data-toggle="tooltip" title="編輯"></i></a>

                                        <form id="del_icon" action="{{ route('DinerType.destroy', $DinerType->id) }}"
                                            method="post" style="display: inline-block;">
                                            @csrf @method('DELETE')

                                            <a href="{{ 'DinerType/delete/' }}{{ $DinerType->id }}" class=" mx-1"
                    ...
                    <div style="width:85%;margin:auto;  class=" mt-1" class="d-flex">
                        <div class="card-body d-flex justify-content-end mr-0">
                            {{ $DinerTypes->appends(['search' => request()->search])->links('vendor.pagination.bootstrap-5') }}
                        </div>
                    </div>
