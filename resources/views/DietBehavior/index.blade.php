            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    ...
                    <div style="width:85%;margin: auto;" class="d-flex justify-content-between align-items-end mb-2 mt-5">
                        <a href="{{ route('DietBehavior.create') }}" class="btn "
                            style="border-radius: 0;background-color: #999;color:#fff">
                            <span class="d-flex align-items-center mx-0"><i class="material-icons">&#xE147;</i><span class="ml-1">新增飲食習性清單</span></span>
                        </a>
                        <form action="{{ route('DietBehavior_search') }}" method="GET" class="">
                        ...
                        <tbody>
                            <tr>
                                @foreach ($DietBehaviors as $DietBehavior)
                                    <td data-label="編號"> {{ $DietBehavior->db_no }}</td>
                                    <td data-label="名稱"> {{ $DietBehavior->db_name }}</td>
                                    @if(trim($DietBehavior->db_type) != null)
                                        <td data-label="類型"> {{ $DietBehavior->db_type }}</td>
                                    @else
                                        <td data-label="類型"> &nbsp; </td>
                                    @endif

                                    {{-- @if ($DietGroup->dg_type != null)
                                    ...
                                    {{-- <td data-label="類型"> {{ $DietBehavior->db_type }}</td> --}}
                                    <td data-label="操作">
                                        <a href="{{ route('DietBehavior.show', $DietBehavior->id) }}" class="show mx-1">
                                            <i class="fa-sharp fa-solid fa-eye " style="color:#36304A;"
                                                data-toggle="tooltip" title="檢視"></i></a>


                                        <a href="{{ route('DietBehavior.edit', $DietBehavior->id) }}" class="edit mx-1">
                                            <i class="fa-solid fa-pen-to-square" style="color:#36304A;"
                                                data-toggle="tooltip" title="編輯"></i></a>

                                        <form id="del_icon"
                                            action="{{ route('DietBehavior.destroy', $DietBehavior->id) }}"
                                            method="post" style="display: inline-block;">
                                            @csrf @method('DELETE')

                                            <a href="{{ 'DietBehavior/delete/' }}{{ $DietBehavior->id }}" class=" mx-1"
                    ...
                    <div style="width:85%;margin:auto;  class=" mt-1" class="d-flex">
                        <div class="card-body d-flex justify-content-end mr-0">
                            {{ $DietBehaviors->appends(['search' => request()->search])->links('vendor.pagination.bootstrap-5') }}
                        </div>
                    </div>
